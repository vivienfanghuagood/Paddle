...
void FusedXFTWeightQuantizeKernel(const Context& dev_ctx,
    ...
    if constexpr (std::is_same_v<T, float>) {
        ...
        if(algo == "weight_only_int8"){
            ...
            quantizedWeight.data.buf = out->mutable_data<int8_t>(cpu_place);
            MMHelper::convertWeight<int8_t>(trans, weight_dims[0], weight_dims[1], weight_data, quantizedWeight, scaleWeight, zeroWeight);
        }
